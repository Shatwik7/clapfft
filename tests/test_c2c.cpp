...
namespace
{
    ...
    template <typename T>
    bool complex_close(const std::vector<std::complex<T>> &expected,
                       ...
                       T eps)
    {
        ...
        {
            if (std::abs(expected[i].real() - actual[i].real()) > eps)
            {
                return false;
            }
            if (std::abs(expected[i].imag() - actual[i].imag()) > eps)
            {
                return false;
            }
        }
    ...
    template <typename T>
    bool real_close(const std::vector<T> &expected,
    ...
    void assert_complex_close(const std::vector<std::complex<T>> &expected,
                              ...
                              T eps)
    {
        assert(complex_close(expected, actual, eps));
    }
    ...
    void assert_real_close(const std::vector<T> &expected,
                           ...
                           T eps)
    {
        assert(real_close(expected, actual, eps));
    }

    template <typename T>
    void test_parallel_same_plan_c2c_1d()
    {
        const int n = 32;
        ...
        for (int t = 0; t < thread_count; ++t)
        {
            workers.emplace_back([&, t]()
                                 {
            std::vector<std::complex<T>> base = make_complex_signal_1d<T>(n);
            for (std::size_t i = 0; i < base.size(); ++i) {
                base[i] += std::complex<T>(static_cast<T>(t) * static_cast<T>(0.01), static_cast<T>(0));
            }

            for (int iter = 0; iter < iterations && ok.load(); ++iter) {
                std::vector<std::complex<T>> spectrum;
                std::vector<std::complex<T>> recovered;

                clapfft::FFT::c2c_1d(base, spectrum, FFTW_FORWARD);
                ...
                if (!complex_close(base, recovered, eps * static_cast<T>(2))) {
                    ok.store(false);
                    break;
                }
            } });
    ...
    template <typename T>
    void test_parallel_same_plan_r2r_1d()
    {
        const int n = 32;
        const int thread_count = 8;
        const int iterations = 40;
        ...
        for (int t = 0; t < thread_count; ++t)
        {
            workers.emplace_back([&, t]()
                                 {
            std::vector<T> base = make_real_signal_1d<T>(n);
            for (std::size_t i = 0; i < base.size(); ++i) {
                base[i] += static_cast<T>(t) * static_cast<T>(0.05);
            }
            ...
            for (int iter = 0; iter < iterations && ok.load(); ++iter) {
                ...
                clapfft::FFT::r2r_1d(base, forward, FFTW_REDFT10);
                ...
                if (!real_close(base, recovered, eps * static_cast<T>(6))) {
                    ok.store(false);
                    break;
                }
            } });
    ...
    {
        test_parallel_same_plan_c2c_1d<T>();
        test_parallel_same_plan_r2r_1d<T>();
